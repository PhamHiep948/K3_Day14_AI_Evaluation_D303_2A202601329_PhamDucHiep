...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        return _clamp01(ap_sum / num_relevant)
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            f"Answer: {answer}\n"
            f"Rubric:\n{rubric_lines}\n"
            "Return JSON mapping each criterion to a score between 0 and 1."
        )
        ...
        try:
            ...
            if isinstance(parsed, dict):
                scores = {
                    name: float(parsed.get(name, 0.5))
                    for name in rubric
                }
                # Keep any extra keys the judge returned that match known criteria
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, failure in enumerate(failures):
            ...
            suggested_fix = suggestions[idx] if idx < len(suggestions) else ""
            lines.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0:
            suggestions.append(
                "Clarify prompts and add intent checks so answers address the question"
            )
        ...
        if categories.get("off_topic", 0) > 0:
            suggestions.append(
                "Tighten intent detection and topic routing to reduce off-topic replies"
            )

        # Ensure at least 3 concrete suggestions when there are failures
        defaults = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Improve retrieval ranking or add reranking to raise context precision",
            "Expand context window and verify expected facts are present in sources",
            "Calibrate evaluation thresholds and add regression gates in CI/CD",
        ]
